...
with DAG(
    ...
    description="Chạy 2 job Spark: customer_scd4a_current và customer_scd4a_hist",
) as dag:

    # 1️⃣ Job tạo bảng current
    job_current = SparkSubmitOperator(
        task_id="bronze_to_silver_customer_scd4a_current",
        application="/opt/bitnami/spark/app/airflow/job/bronze2sil_scd4a_current_customer.py",
    ...
    # 2️⃣ Job tạo bảng hist
    job_hist = SparkSubmitOperator(
        task_id="bronze_to_silver_customer_scd4a_hist",
        application="/opt/bitnami/spark/app/airflow/job/bronze2sil_scd4a_hist_customer.py",
        py_files="/opt/bitnami/spark/app/airflow/core.zip",
        conn_id="spark_default",
        packages="org.postgresql:postgresql:42.7.3",
        jars="/opt/airflow/jars/hadoop-aws-3.3.4.jar,"
